class FormHandler {
    ...
    editForm(id, token, area = "") {
        ...
        if (id > 0) {
            this.sendAjaxRequest(null, 'GET', `http://localhost:5043/api/LicenseApplications/${id}`, this.handleEditFormSuccess.bind(this), this.handleError.bind(this), {
    ...
    delete(id, token) {
        bootbox.confirm("Are you sure you want to delete this application from the system?", result => {
            if (result) {
                this.sendAjaxRequest(null, 'DELETE', `http://localhost:5043/api/licenseapplication/${id}`, this.handleDeleteSuccess.bind(this), this.handleError.bind(this), {
                    'Authorization': `Bearer ${token}`
                });
            }
        });
    ...
    updateApplication(token) {
        ...
        this.sendAjaxRequest(formData, 'PUT', `http://localhost:5043/api/licenseapplication/${id}`, this.handleUpdateApplicationSuccess.bind(this), this.handleError.bind(this), {
            'Authorization': `Bearer ${token}`
        });
    }
