class SubcommitteeMessageHandler {
    constructor() {
        this.form = document.querySelector("#subcommitteeChatForm")
    ...
    bindEvents() {
        ...
        if (createMessageBtn) {
            ...
            console.log("createMessageBtn is null")
        }
        ...
        if (loadMoreMessagesBtn) {
            loadMoreMessagesBtn.addEventListener("click", () => {
                ...
                this.getChatMessages(subcommitteeThreadId, 5, skip)
            })
        }
    ...
    updateClicked() {
        ...
        if (!form.checkValidity()) {
            ...
            this.sendAjaxRequest(
                formData,
                "PUT",
                `${host}/api/SubcommitteeMessages/${id}`,
    ...
    editForm(id, token) {
        if (id > 0) {
            this.sendAjaxRequest(null, 'GET', `${host}/api/SubcommitteeMessages/GetMessageById/${id}`, this.handleEditFormSuccess.bind(this), this.handleError.bind(this), {
    ...
    delete(event) {
        ...
        bootbox.confirm("Are you sure you want to delete this subcommittee message?", result => {
            if (result) {
                this.sendAjaxRequest(null, 'DELETE', `${host}/api/SubcommitteeMessages/${id}`, this.handleDeleteSuccess.bind(this), this.handleError.bind(this, null), {
                    'Authorization': `Bearer ${token}`
                })
            }
        })
    ...
    handleUpdateSuccess(response) {
        toastr.success("Subcommittee message updated successfully")
    }

    handleDeleteSuccess(response) {
        toastr.success("Subcommittee message has been deleted successfully")
    }

    getChatMessages(threadId, numMessages, skip = 0) {
        const url = `${host}/api/SubcommitteeMessages/GetSubcommitteeMessageByRange/${threadId}/${numMessages}/${skip}`
        const messagesList = document.getElementById("messagesList")
...
document.addEventListener("DOMContentLoaded", () => {
    const subcommitteeMessageHandler = new SubcommitteeMessageHandler()
})
